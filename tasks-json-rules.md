...
Тип поддерживает слова и выражения, содержащие пробелы. Пробел добавляется как отдельный символ в массив `letters`.

Пример с пробелом:
```json
{
  "type": "buildForeignWord",
  "prompt": "Собери английское выражение",
  "sourceWord": "мороженое",
  "targetWord": "ice cream",
  "letters": ["i", "c", "e", " ", "c", "r", "e", "a", "m", "s", "t"],
  "hint": "**ice cream** — это __мороженое__."
}
```

Отображение:
- Пробел в `targetWord` создаёт визуальный разрыв между словами
- На кнопке пробел отображается как символ подчёркивания `_` (для видимости)
- Пользователь нажимает на кнопку `_` так же, как на любую другую букву

Правила валидации:
1. `sourceWord` — непустая строка.
2. `targetWord` — непустая строка.
3. `letters` — массив непустых строк (обычно длины 1, но символ `" "` также допустим).
4. `letters` содержит все символы `targetWord` с нужной кратностью (включая пробелы).
...
Режим также поддерживает слова с пробелами, аналогично `buildForeignWord`:

```json
{
  "type": "audioToWord",
  "mode": "buildWord",
  "prompt": "Прослушай и собери английское выражение",
  "audioSrc": "audio/en/baggage.mp3",
  "targetWord": "baggage compartment",
  "letters": ["b", "a", "g", "g", "a", "g", "e", " ", "c", "o", "m", "p", "a", "r", "t", "m", "e", "n", "t"],
  "hint": "**baggage compartment** — багажное отделение."
}
```

Правила:
1. `audioSrc` — непустая строка.
2. `targetWord` и `letters` валидируются так же, как в `buildForeignWord` (включая поддержку пробелов).
