...
3. `prompt` — опциональная строка (текст инструкции в интерфейсе).

## Формат `hint`
`hint` поддерживает встроенный легкий формат:
1. `**жирный**`
2. `__подчеркнутый__`
3. переносы строк
4. ударение: символ `` ` `` после ударной гласной

Пример:
- `моло`чный` отображается как `моло́чный`.
